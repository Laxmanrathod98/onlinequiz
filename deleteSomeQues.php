	...
	if(isset($_POST['total_ques']) && $_POST['total_ques'] != ""){
		...
		for($i=1 ; $i <= $total_questions ; $i++) {
	        ...
	        if(isset($_POST[$fetch_ID])) {
				...
				if($php_id){
					$check_sql = $con->query("SELECT quiz_id FROM questions 
	                                            WHERE question_id='$php_id'") or die("mysql_error()");
					$qz_id_array = $check_sql->num_rows;
	            	$qz_id = $qz_id_array[0];

					$con->query("DELETE FROM questions 
									WHERE question_id='$php_id'")or die("mysql_error()");
					$con->query("DELETE FROM answers 
									WHERE question_id='$php_id'")or die("mysql_error()");

					$con->query("UPDATE quizes SET total_questions=total_questions-1 
