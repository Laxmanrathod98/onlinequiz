<?php
	...
	if(isset($_POST['desc'])){
		...
		$sql = $con->query("INSERT INTO questions (quiz_id, question, code, code_type, type) VALUES ('$quizID', '$question', '$program', '$programType', '$type')")or die("mysql_error()");
		
			$lastId = $con->insert_id;
			$con->query("UPDATE questions SET question_id='$lastId' WHERE id='$lastId' LIMIT 1")or die("mysql_error()");

	 
		$con->query("UPDATE quizes SET total_questions=total_questions+1 WHERE quiz_id='$quizID' LIMIT 1")or die("mysql_error()");
		...
		if($type == 'tf'){
			...
			if($isCorrect == "answer1"){
				$sql2 = $con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer1', '1')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer2', '0')")or die("mysql_error()");
			...
			if($isCorrect == "answer2"){
				$sql2 = $con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer2', '1')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer1', '0')")or die("mysql_error()");
				$msg = 'Thanks, question no.'.$lastId.' has been added';
				header('location: admin.php?msg='.$msg.'');
				exit();
			}	
		...
		if($type == 'mc'){
			...
			if($isCorrect == "answer1"){
				$sql2 = $con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer1', '1')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer2', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer3', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer4', '0')")or die("mysql_error()");
				$msg = 'Thanks, question no.'.$lastId.' has been added';
			  	header('location: admin.php?msg='.$msg.'');
				exit();
			}
			...
			if($isCorrect == "answer3"){
				$sql2 = $con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer3', '1')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer1', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer2', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer4', '0')")or die("mysql_error()");
				$msg = 'Thanks, question no.'.$lastId.' has been added';
			...
			if($isCorrect == "answer4"){
				$sql2 = $con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer4', '1')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer1', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer2', '0')")or die("mysql_error()");
				$con->query("INSERT INTO answers (quiz_id, question_id, answer, correct) VALUES ('$quizID', '$lastId', '$answer3', '0')")or die("mysql_error()");
				$msg = 'Thanks, question no.'.$lastId.' has been added';
...
<?php
	...
	if(isset($_POST['resetTables']) && $_POST['resetTables'] != ""){
		...
		if($resetT=='yes'){
			$con->query("TRUNCATE TABLE admins")or die("mysql_error()");
			$con->query("TRUNCATE TABLE answers")or die("mysql_error()");
			$con->query("TRUNCATE TABLE questions")or die("mysql_error()");
			$con->query("TRUNCATE TABLE quizes")or die("mysql_error()");
			$con->query("TRUNCATE TABLE quiz_takers")or die("mysql_error()");
...
<?php
	...
	if(isset($_POST['editaquestion']) && $_POST['editaquestion'] != ""){
		...
		$get_quiz_id_SQL = $con->query("SELECT quiz_id FROM quizes 
										WHERE quiz_name = '$editQ'")or die("mysql_error()");
		$get_quiz_id_rows = $get_quiz_id_SQL->fetch_assoc();
		$get_quiz_id = $get_quiz_id_rows['quiz_id'];

		$m_output='';

		if($editQ=='allthequestions')
			$multipleSQL = $con->query("SELECT * FROM questions") or die("mysql_error()");
		else
			$multipleSQL = $con->query("SELECT * FROM questions WHERE quiz_id = '$get_quiz_id'");

			$m_display_ID = 1;

			while($m_row = $multipleSQL->fetch_assoc()){
				$m_answers='';
				...
				$m_quiz_id_SQL = $con->query("SELECT quiz_name FROM quizes 
												WHERE quiz_id='$m_quiz_id'") or die("mysql_error()");
				$m_quiz_id_SQL_row = $m_quiz_id_SQL->fetch_assoc();
				...
				$m_sql2 = $con->query("SELECT * FROM answers WHERE question_id='$m_question_id'") or die("mysql_error()");
				...
				$m_answers .=  '<tr>
					...
					while($m_row2 = $m_sql2->fetch_assoc()){
...
<?php
	...
	if(isset($_POST['editAQ']) && $_POST['editAQ'] != ""){
		...
		$getaquestion_SQL = $con->query("SELECT * FROM questions 
						WHERE question_id='$editAQ'")or die("mysql_error()");
		$getaquestion_row = $getaquestion_SQL->fetch_assoc();
		...
		$gaq_code_editor = $con->escape_string($gaq_code_editor);
		$gaq_code_editor = str_replace(array("\r\n", "\r", "\n"), '\n', $gaq_code_editor);



		$getanswers_SQL = $con->query("SELECT * FROM answers 
						WHERE question_id='$editAQ'")or die("mysql_error()");
		...
		if($gaq_type=='tf'){
			...
			$ga_index=1;
			while($getanswers_row = $getanswers_SQL->fetch_assoc()){
			...
			while($getanswers_row = $getanswers_SQL->fetch_assoc()){
				$ga_answer = $getanswers_row['answer'];
				$ga_correct = $getanswers_row['correct'];
				$ga_answer = $con->escape_string($ga_answer);
...
<?php 
	...
	if(isset($_POST['deleteAdmin']) && $_POST['deleteAdmin'] != ""){
		...
		$con->query("DELETE FROM admins WHERE username = '$deleteA'")or die("mysql_error()");

	
		$admin_SQL = $con->query("SELECT id FROM admins 
...
<?php 
	...
	if(isset($_POST['defaultQuiz']) && $_POST['defaultQuiz'] != ""){
		...
		$con->query("UPDATE quizes SET set_default=0 WHERE set_default=1")or die("mysql_error()");
		$con->query("UPDATE quizes SET set_default=1 WHERE quiz_name='$defaultQ'")or die("mysql_error()");


 
	
		$defaultQ_sql1 = $con->query("SELECT id FROM quizes WHERE set_default=1")or die("mysql_error()");
...
<?php 
	...
	if(isset($_POST['clearResult']) && $_POST['clearResult'] != ""){
		...
		$con->query("DELETE FROM quiz_takers WHERE quiz_id='$clearR'")or die("mysql_error()");



 
	
		$QuizTakersSQL = $con->query("SELECT id FROM quiz_takers WHERE quiz_id='$clearR' LIMIT 1")or die("mysql_error()");
...
<?php 
	...
	if(isset($_POST['reset']) && $_POST['reset'] != ""){
		...
		$con->query("UPDATE quizes SET total_questions=0")or die("mysql_error()");


 
	
		$sql1 = $con->query("SELECT id FROM questions LIMIT 1")or die("mysql_error()");
		$sql2 = $con->query("SELECT id FROM answers LIMIT 1")or die("mysql_error()");
...
<?php 
	...
	if(isset($_POST['deleteQuiz']) && $_POST['deleteQuiz'] != ""){
		...
		$qz_id_SQL = $con->query("SELECT quiz_id FROM quizes 
									WHERE quiz_name = '$deleteQ'")or die("mysql_error()");
		$qz_id_SQL_row = $qz_id_SQL->fetch_assoc();
        $qz_id = $qz_id_SQL_row['quiz_id'];

		$con->query("DELETE FROM quizes WHERE quiz_id = '$qz_id'")or die("mysql_error()");
		$con->query("DELETE FROM questions WHERE quiz_id = '$qz_id'")or die("mysql_error()");
		$con->query("DELETE FROM answers WHERE quiz_id = '$qz_id'")or die("mysql_error()");

 
	
		$qz_sql1 = $con->query("SELECT id FROM questions WHERE quiz_id = '$qz_id' LIMIT 1")or die("mysql_error()");
		$qz_sql2 = $con->query("SELECT id FROM answers WHERE quiz_id = '$qz_id' LIMIT 1")or die("mysql_error()");
		$qz_sql3 = $con->query("SELECT id FROM quizes WHERE quiz_id = '$qz_id' LIMIT 1")or die("mysql_error()");
...
<?php
	...
	$quizIdSQL = $con->query("SELECT quiz_id, quiz_name, display_questions, time_allotted FROM quizes") or die("mysql_error()");

	 
		while($quizID_row = $quizIdSQL->fetch_assoc()){
...
<?php
	...
	if(isset($_POST['usersQuiz']) && $_POST['usersQuiz'] != ""){
		...
		$multipleSQL = $con->query("SELECT * FROM quiz_takers 
